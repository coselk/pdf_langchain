...
import config,fastapi,uvicorn,logging,sys,torch
...
import os
os.environ['TRANSFORMERS_CACHE'] = '/Users/lucien/.cache/huggingface/hub'
# get-logger用于记录日志
...
class ChatGLM():
    def __init__(self) -> None:
        ...
        self.model = AutoModel.from_pretrained(config.LLM_MODEL_PATH,trust_remote_code=True).float()
        self.device = 'cpu'  # if no GPU, set "cpu"
...
def start_server():
    ...
    host = env.get("HOST") if env.get("HOST") is not None else "127.0.0.1"
